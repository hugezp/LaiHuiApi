        ...
        try {
            ...
            switch (action) {
                ...
                case "nearby_passenger":
                    //符合要求的乘客列表
                    where = " where is_enable =1 and order_status = 0 and a.user_id != " + user_id + " and departure_time >'" + current_time + "' having s_distance <= " + query_distance + " and e_distance <= " + query_distance + " order by convert (departure_time USING gbk)COLLATE gbk_chinese_ci asc limit " + offset + " ,"+size;
