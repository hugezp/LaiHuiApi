    ...
    @ResponseBody
     ...
     public ResponseEntity<String> pop_up_ad(HttpServletRequest request) {
        ...
        try {
            ...
            switch (action) {
                case "show":
                    json = AppJsonUtils.returnSuccessJsonString(AppJsonUtils.getPopUpAdJson(appDB,0), "弹出广告获取成功");
                    return new ResponseEntity<>(json, responseHeaders, HttpStatus.OK);
                case "show_list":
                    json = AppJsonUtils.returnSuccessJsonString(AppJsonUtils.getPopUpAdJson(appDB,1), "弹出广告获取成功");
                    return new ResponseEntity<>(json, responseHeaders, HttpStatus.OK);
            }
            json = AppJsonUtils.returnFailJsonString(result, "获取参数错误");
            return new ResponseEntity<>(json, responseHeaders, HttpStatus.OK);
